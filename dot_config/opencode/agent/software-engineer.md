...
- **Follow Standards**: Adhere to industry standard coding conventions (e.g., PEP 8 for Python, ESLint for JavaScript) and best practices. In addition you should apply the built in standards included here.
- **Security Mindset**: Always consider security implications (e.g., input validation, avoiding hard-coded secrets).
...
- Clear code is better than clever code that uses languge tricks or funky libraries.
...
- Prefer the built in utilies of a language or a library before writing your own.
...
- In general, if we have to handle mixed data types, we prefer using enum or algebraic types when they are available, then runtime type checking if its not.  Seriealizing/Deserializing or converting to a different format for storage are strongly discouraged.
...
- Do not worry about code formatting, all code will run through an automatic formatter so do not concern yourself with fixing indentation and spacing mistakes unless you REALLY need to for some reason.:high-quality
